...
## by Make
### **Monthly Vehicles Registrations by Make**
...
```
### **Monthly Trends for Top Brands**

```{r, echo=FALSE, message=FALSE}
# --- Identify the top brands from your YTD summary ---
...
month_current_count_fuel <- car_data |>
  filter(date_reg == month_current) |>
  count(fuel, name = "count_current_fuel")

month_previous_count_fuel <- car_data |>
  filter(date_reg == month_previous) |>
  count(fuel, name = "count_previous_fuel")

month_previous_year_count_fuel <- car_data |>
  filter(date_reg == month_previous_year) |>
  count(fuel, name = "count_previous_year_fuel")

ytd_current_count_fuel <- car_data |>
    filter(date_reg >= ytd_current_start & date_reg <= ytd_current_end) |>
    count(fuel, name = "count_ytd_current_fuel")

ytd_previous_count_fuel <- car_data |>
    filter(date_reg >= ytd_previous_start & date_reg <= ytd_previous_end) |>
    count(fuel, name = "count_ytd_previous_fuel")

# --- Join and calculate MoM growth for fuel type ---
summary_data_fuel <- month_current_count_fuel |>
  full_join(month_previous_count_fuel, by = 'fuel') |>
  full_join(month_previous_year_count_fuel, by = "fuel") |>
  full_join(ytd_current_count_fuel, by = "fuel") |>
  full_join(ytd_previous_count_fuel, by = "fuel") |>
  mutate(
  ...
  mutate(rank = row_number()) |>
  select(rank, fuel, count_current_fuel, count_previous_fuel, growth_MoM_fuel, count_previous_year_fuel, growth_YoY_fuel, count_ytd_current_fuel, count_ytd_previous_fuel, growth_YTD_fuel)
```
...
datatable(table_display_fuel,
          rownames = FALSE,
          filter = 'top',
          # caption = 'Monthly Vehicles Registrations by Fuel Type',
          options = list(pageLength = 100, order = list(0, 'asc')),
          class = 'cell-border stripe') |>
  formatPercentage("Growth (MoM)", 1) |>
  formatPercentage("Growth (YoY)", 1) |>
