...
Column
-----------------------------------------------------------------------
### **Monthly Registration Trend for Top 10 Brands**
...
month_current_count_model <- car_data |>
  filter(date_reg == month_current) |>
  count(maker, model, fuel_grouped, name = "count_current_model")

month_previous_count_model <- car_data |>
  filter(date_reg == month_previous) |>
  count(maker, model, fuel_grouped, name = "count_previous_model")

month_previous_year_count_model <- car_data |>
  filter(date_reg == month_previous_year) |>
  count(maker, model, fuel_grouped, name = "count_previous_year_model")

ytd_current_count_model <- car_data |>
    filter(date_reg >= ytd_current_start & date_reg <= ytd_current_end) |>
    count(maker, model, fuel_grouped, name = "count_ytd_current_model")

ytd_previous_count_model <- car_data |>
    filter(date_reg >= ytd_previous_start & date_reg <= ytd_previous_end) |>
    count(maker, model, fuel_grouped, name = "count_ytd_previous_model")

# --- Join and calculate MoM growth ---
summary_data_model <- month_current_count_model |>
  full_join(month_previous_count_model, by = c("maker", "model", "fuel_grouped")) |>
  full_join(month_previous_year_count_model, by = c("maker", "model", "fuel_grouped")) |>
  full_join(ytd_current_count_model, by = c("maker", "model", "fuel_grouped")) |>
  full_join(ytd_previous_count_model, by = c("maker", "model", "fuel_grouped")) |>
  mutate(
  ...
  mutate(rank = row_number()) |>
  select(rank, maker, model, fuel_grouped, count_current_model, count_previous_model, growth_MoM, count_previous_year_model, growth_YoY,
...
Column
-----------------------------------------------------------------------
### **Monthly Registration Trend for Top 10 Models**
