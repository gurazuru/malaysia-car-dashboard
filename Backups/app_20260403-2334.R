...
ui <- page_fluid(
  ...
  card(
    height = c(160),
    card_header("data as of 28th February 2026"),
...
server <- function(input, output, session) {
  ...
  make_summary <- function(data, group_col, group_col_name) {
    ...
    if (group_col == "model"){
      ...
      model_list() |>
        ...
        select(rank, maker, !!group_col, count_current, count_previous, growth_MoM, count_previous_year, growth_YoY, count_ytd_current, count_ytd_previous, growth_YTD, total_count) |>
        rename(
          `Rank` := rank,
          `Make` := maker,
  ...
  make_annual <- function(data, group_col, group_col_name) {
    ...
    Jan_Count <- monthly_data(1, 2025)
    Feb_Count <- monthly_data(2, 2025)
    Mar_Count <- monthly_data(3, 2025)
    Apr_Count <- monthly_data(4, 2025)
    May_Count <- monthly_data(5, 2025)
    Jun_Count <- monthly_data(6, 2025)
    Jul_Count <- monthly_data(7, 2025)
    Aug_Count <- monthly_data(8, 2025)
    Sep_Count <- monthly_data(9, 2025)
    Oct_Count <- monthly_data(10, 2025)
    Nov_Count <- monthly_data(11, 2025)
    Dec_Count <- monthly_data(12, 2025)
    ...
    year_colname <- paste0("Total ", format(as.Date(paste(2025, 1, 1, sep = "-")), "%Y"))
  ...
  plot_chart <- function(df, group_col, group_col_name, agg_choice) {
    df <- fuelfiltered_data() # call the reactive to get the data frame.
